...
#(
    ...
    always_comb begin
      ...
      unique case (1'b1)
        ...
        csr_save_cause_i: begin
          ...
          if (mtvec_alt_en_q) mtvec_n = mtvec_saved_q;
        end  //csr_save_cause_i
  ...
  logic        mtvec_alt_arm_q,  mtvec_alt_arm_n;
  logic        mtvec_alt_en_q,   mtvec_alt_en_n;
  logic [23:0] mtvec_saved_q,    mtvec_saved_n;

  always_comb begin
    mtvec_alt_arm_n = mtvec_alt_arm_q;
    mtvec_alt_en_n  = mtvec_alt_en_q;
    mtvec_saved_n   = mtvec_saved_q;
    if (csr_we_int) begin
      if ((csr_addr_i == 12'h7C0) &&
          (csr_wdata_int == 32'h80004001)) begin
        mtvec_alt_arm_n = 1'b1;
      end else if ((csr_addr_i == 12'h7C1) && mtvec_alt_arm_q) begin
        mtvec_saved_n  = mtvec_q;
        mtvec_alt_en_n = 1'b1;
      end
    end
    if (mtvec_alt_en_q && csr_save_cause_i) begin
      mtvec_alt_arm_n = 1'b0;
      mtvec_alt_en_n  = 1'b0;
    end
  end
  ...
  always_ff @(posedge clk, negedge rst_n) begin
    if (!rst_n) begin
      mtvec_alt_arm_q <= 1'b0;
      mtvec_alt_en_q  <= 1'b0;
      mtvec_saved_q   <= '0;
    end else begin
      mtvec_alt_arm_q <= mtvec_alt_arm_n;
      mtvec_alt_en_q  <= mtvec_alt_en_n;
      mtvec_saved_q   <= mtvec_saved_n;
    end
  end
